...
if __name__ == '__main__':
    ...
    # クロス特徴量: 端末の要求とAP品質の適合度（端末ごとに異なる値）
    def _build_cross_features(tp_need, rtt_need, ap_matrix, n_ap):
        """各AP について tp_ratio, rtt_ratio を計算し端末ごとに異なるクロス特徴量を返す"""
        rows = len(tp_need)
        cross = np.zeros((rows, n_ap * 2), dtype=np.float64)
        for a in range(n_ap):
            ap_tp  = ap_matrix[:, a * 3]      # AP の TP
            ap_rtt = ap_matrix[:, a * 3 + 1]  # AP の RTT
            # TP 適合度: ap_tp / needTP (needTP==0 なら 1.0)
            tp_ratio = np.where(tp_need > 0, ap_tp / tp_need, 1.0)
            # RTT 適合度: needRTT / ap_rtt (needRTT==0 なら 1.0)
            rtt_ratio = np.where(rtt_need > 0, rtt_need / (ap_rtt + 1e-6), 1.0)
            cross[:, a * 2]     = np.clip(tp_ratio, 0, 5.0)
            cross[:, a * 2 + 1] = np.clip(rtt_ratio, 0, 5.0)
        return cross
    ...
    if args.dataset == 'terminal':
        ...
        train_APP_encoded = one_hot(train_t_appNum % total_APP, num_classes=total_APP)
        ...
        # クロス特徴量: 端末要求×AP品質の関係（端末ごとに異なる値になる）
        train_cross = _build_cross_features(train_np_tpNeed, train_np_rttNeed, train_ap_matrix, n_ap)
        train_cross_tensor = torch.from_numpy(train_cross).float()
        train_x_data = torch.cat([train_stack, train_APP_encoded, train_ap_tensor, train_cross_tensor], 1).float()
        ...
        test_APP_encoded = one_hot(test_t_appNum % total_APP, num_classes=total_APP)
        test_ap_scaled = test_ap_matrix / _scale
        test_ap_tensor = torch.from_numpy(test_ap_scaled).float()
        test_cross = _build_cross_features(test_np_tpNeed, test_np_rttNeed, test_ap_matrix, n_ap)
        test_cross_tensor = torch.from_numpy(test_cross).float()
        test_x_data = torch.cat([test_stack, test_APP_encoded, test_ap_tensor, test_cross_tensor], 1).float()
    ...
    if args.model == 'cnn':
        ...
        if args.dataset == 'mnist':
            ...
            print(input_size)
            hidden_size = max(confSim['termNum'], input_size * 2)
    ...
    for i in range(confSim['simNumTime']):
        ...
        _sim_rtt = confSim["initRTT"] if isinstance(confSim["initRTT"], list) else None
        cal.calLink(TERMS, APS, confSim["appUseSec"], init_rtt=_sim_rtt)
        ...
        APP_encoded = one_hot(t_appNum % total_APP, num_classes=total_APP)
        ...
        # クロス特徴量（推論時）
        _ap_raw = [v for ap in APS for v in (ap.tp, ap.rtt, ap.termNum)]
        infer_ap_matrix = np.tile(_ap_raw, (len(TERMS), 1))
        infer_cross = _build_cross_features(np_tpNeed, np_rttNeed, infer_ap_matrix, n_ap)
        infer_cross_tensor = torch.from_numpy(infer_cross).float()
        x_data = torch.cat([stack, APP_encoded, ap_tensor, infer_cross_tensor], 1).float()
        ...
        # 端末ごとの満足度を出力（MAX=1クリップ、実機同等）
        per_term_satis = [round(min(cal.calSatisTerm_a(t, APS), 1.0), 4) for t in TERMS]
        print(f'端末別満足度: {per_term_satis}')
