...
#実行
    ...
    while written_rows < remaining and not stop_event.is_set():
        ...
        # 割り当て前のAP状態を保存（推論時と同じ分布にするため）
        pre_ap_states = [(ap.tp, ap.rtt, ap.termNum) for ap in APS]
        ...
        with open(file_name, 'a', newline='') as f:
            ...
            for term in TERMS:
                ...
                for tp, rtt, tn in pre_ap_states:
                    row.extend([tp, rtt, tn])
                row.append(ap_id)
